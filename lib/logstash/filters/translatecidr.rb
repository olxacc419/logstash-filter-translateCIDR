...
class TranslateCIDR < LogStash::Filters::Base
  ...
  def register
    ...
    if @dictionary_path
      @lookup = Dictionary::File.create(@dictionary_path, @refresh_interval, @refresh_behaviour, @exact, @regex, @cidr)
    else
      @lookup = Dictionary::Memory.new(@dictionary, @exact, @regex, @cidr)
    end
    if @iterate_on.nil?
