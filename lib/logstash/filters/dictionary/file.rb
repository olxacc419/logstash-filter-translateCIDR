...
module LogStash module Filters module Dictionary
  ...
  class File
    def self.create(path, refresh_interval, refresh_behaviour, exact, regex, cidr)
      if /\.y[a]?ml$/.match(path)
        instance = YamlFile.new(path, refresh_interval, exact, regex, cidr)
      elsif path.end_with?(".json")
        instance = JsonFile.new(path, refresh_interval, exact, regex, cidr)
      elsif path.end_with?(".csv")
        instance = CsvFile.new(path, refresh_interval, exact, regex, cidr)
      else
    ...
    def initialize(path, refresh_interval, exact, regex, cidr)
      ...
      klass = case
              ...
              when exact && cidr  then FetchStrategy::File::Cidr
